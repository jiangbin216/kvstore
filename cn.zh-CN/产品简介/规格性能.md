...
|--|----------------------|--------|------------|--------|------|
|读写分离1GB版（1节点，每节点1只读）|redis.logic.splitrw.small.1db.1rodb.4proxy.default|1|192|20000|20万|
|读写分离1GB版（1节点，每节点3只读）|redis.logic.splitrw.small.1db.3rodb.4proxy.default|3|384|40000|40万|
|读写分离1GB版（1节点，每节点5只读）|redis.logic.splitrw.small.1db.5rodb.6proxy.default|5|576|60000|60万|
|读写分离2G版（1节点，每节点1只读）|redis.logic.splitrw.mid.1db.1rodb.4proxy.default|1|192|20000|20万|
|读写分离2G版（1节点，每节点3只读）|redis.logic.splitrw.mid.1db.3rodb.4proxy.default|3|384|40000|40万|
|读写分离2G版（1节点，每节点5只读）|redis.logic.splitrw.mid.1db.5rodb.6proxy.default|5|576|60000|60万|
|读写分离4G版（1节点，每节点1只读）|redis.logic.splitrw.stand.1db.1rodb.4proxy.default|1|192|20000|20万|
|读写分离4G版（1节点，每节点3只读）|redis.logic.splitrw.stand.1db.3rodb.4proxy.default|3|384|40000|40万|
|读写分离4G版（1节点，每节点5只读）|redis.logic.splitrw.stand.1db.5rodb.6proxy.default|5|576|60000|60万|
|读写分离8G版（1节点，每节点1只读）|redis.logic.splitrw.large.1db.1rodb.4proxy.default|1|192|20000|20万|
|读写分离8G版（1节点，每节点3只读）|redis.logic.splitrw.large.1db.3rodb.4proxy.default|3|384|40000|40万|
|读写分离8G版（1节点，每节点5只读）|redis.logic.splitrw.large.1db.5rodb.6proxy.default|5|576|60000|60万|
|读写分离16G版（1节点，每节点1只读）|redis.logic.splitrw.2xlarge.1db.1rodb.4proxy.default|1|192|20000|20万|
|读写分离16G版（1节点，每节点3只读）|redis.logic.splitrw.2xlarge.1db.3rodb.4proxy.default|3|384|40000|40万|
|读写分离16G版（1节点，每节点5只读）|redis.logic.splitrw.2xlarge.1db.5rodb.6proxy.default|5|576|60000|60万|
|读写分离32G版（1节点，每节点1只读）|redis.logic.splitrw.4xlarge.1db.1rodb.4proxy.default|1|192|20000|20万|
|读写分离32G版（1节点，每节点3只读）|redis.logic.splitrw.4xlarge.1db.3rodb.4proxy.default|3|384|40000|40万|
|读写分离32G版（1节点，每节点5只读）|redis.logic.splitrw.4xlarge.1db.5rodb.6proxy.default|5|576|60000|60万|
|读写分离64G版（1节点，每节点1只读）|redis.logic.splitrw.8xlarge.1db.1rodb.4proxy.default|1|192|20000|20万|
|读写分离64G版（1节点，每节点3只读）|redis.logic.splitrw.8xlarge.1db.3rodb.4proxy.default|3|384|40000|40万|
|读写分离64G版（1节点，每节点5只读）|redis.logic.splitrw.8xlarge.1db.5rodb.6proxy.default|5|576|60000|60万|

## 集群版读写分离规格 {#section_dy2_g5g_vgb .section}

|规格|InstanceClass （API 使用）|节点数（个）|只读节点个数|内网带宽上限（MB/s）|连接数上限（个）|QPS参考值|
|--|----------------------|------|------|------------|--------|------|
|读写分离4G版（2节点，每节点1只读）|redis.logic.splitrw.sharding2g.2db.1rodb.4proxy.default|2|1|384|30000|30万|
|读写分离8G版（2节点，每节点1只读）|redis.logic.splitrw.sharding4g.2db.1rodb.4proxy.default|2|1|384|30000|30万|
|读写分离16G版（2节点，每节点1只读）|redis.logic.splitrw.sharding8g.2db.1rodb.8proxy.default|2|1|384|30000|30万|
|读写分离32G版（8节点，每节点1只读）|redis.logic.splitrw.sharding4g.8db.1rodb.16proxy.default|8|1|1536|90000|90万|
|读写分离64G版（16节点，每节点1只读）|redis.logic.splitrw.sharding4g.16db.1rodb.32proxy.default|16|1|3072|170000|170万|
|读写分离128G版（16节点，每节点1只读）|redis.logic.splitrw.sharding8g.16db.1rodb.32proxy.default|16|1|3072|170000|170万|
|读写分离256G版（32节点，每节点1只读）|redis.logic.splitrw.sharding8g.32db.1rodb.64proxy.default|32|1|6144|330000|330万|
|读写分离512G版（32节点，每节点1只读）|redis.logic.splitrw.sharding16g.32db.1rodb.64proxy.default|32|1|6144|330000|330万|
